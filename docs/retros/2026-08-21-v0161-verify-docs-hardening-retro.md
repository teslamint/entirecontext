...
| T5 | 2 | 4 | 2026-08-17 retro's 5 registered items: current status? RELEASE.md checklist applies to patch releases? | 1 Done (verify-docs), 1 In progress (maturity 75), 3 Not started (py.typed, P2 review discipline, plan schema). Patch release had no planning phase; carry-forward review step was skipped. | ROADMAP.md:363-364, 383; ADR 0019; no new Plans authored since 2026-08-17 | no verdict returned (facilitator dispatch timeout after answer submission) |
| T6 | 2 | 3 | b9d1481 mixes doc + CLI change — same scope? | Both respond to CodeRabbit PR #235 feedback. Commit message lists 3 items explicitly. Split is reviewer-response vs. proactive hardening (e6b8e1b). 2-line doc + 3-line CLI fix; splitting adds friction without safety benefit. | `git show b9d1481 --stat` → 2 files; commit message enumerates doc, cli, skip | no verdict returned (facilitator dispatch timeout after answer submission) |
