...
Steps:
  1. Add `test_enable_codex_skips_claude_and_git_hooks` to `tests/test_project_cmds.py`, in the class that already holds `test_enable_codex_writes_user_notify` (around line 393). Follow that test's setup exactly: decorate with `@patch("entirecontext.core.project.find_git_root")`, create `repo = tmp_path / "repo"` with `(repo / ".git" / "hooks").mkdir(parents=True)`, set `mock_git_root.return_value = str(repo)`, and `monkeypatch.setenv("HOME", str(fake_home))` with `fake_home = tmp_path / "fakehome"`. Call `project_cmds.enable(agent="codex")`. Assert: `not (repo / ".claude" / "settings.local.json").exists()`; `not (repo / ".git" / "hooks" / "post-commit").exists()`; `not (repo / ".git" / "hooks" / "pre-push").exists()`; and `"entirecontext" in json.loads((fake_home / ".claude" / "settings.json").read_text())["mcpServers"]`.
  2. Run `uv run pytest tests/test_project_cmds.py::<class>::test_enable_codex_skips_claude_and_git_hooks -x`. Confirm it PASSES. This is a characterization test: it records today's behavior before the refactor, so a failure here means the assertion was written wrong, not that the code is broken.
...
Test scenarios:
  ...
  error: with `_install_integrations` patched to raise `OSError("boom")`, `init()` returns without raising `typer.Exit`, and the captured output contains both the failure text and the string `ec enable` (Covers S6)
...
Steps:
  ...
  2. Write these seven failing tests, all calling `project_cmds.init(...)`: `test_init_installs_hooks_by_default` (all five hook names present in `.claude/settings.local.json`); `test_init_installs_git_hooks_by_default` (`post-commit` and `pre-push` exist and contain `EntireContext`); `test_init_registers_mcp_server` (`mcpServers.entirecontext` present in `fake_home/.claude/settings.json`); `test_init_no_hooks_skips_installation` (with `no_hooks=True`: no `.claude/settings.local.json`, no git hooks, no `fake_home/.claude/settings.json`); `test_init_no_git_hooks_flag` (Claude hooks present, git hooks absent); `test_init_agent_codex_skips_claude_and_git_hooks` (with `agent="codex"`: `~/.codex/config.toml` contains the notify entry, `.claude/settings.local.json` absent, git hooks absent, MCP entry present); `test_init_idempotent` (two `init()` calls leave `len(hooks["SessionStart"]) == 1`).
  3. Run `uv run pytest tests/test_project_cmds.py -k init`. Confirm all seven fail because `init()` takes no arguments — `TypeError: init() got an unexpected keyword argument`.
  ...
  7. Write the eighth test, `test_init_hook_failure_warns_and_exits_zero`: patch `entirecontext.cli.project_cmds._install_integrations` with `side_effect=OSError("boom")`, call `project_cmds.init()`, and assert no exception propagates and that `capsys.readouterr().out` contains both `boom` and `ec enable`. Set `monkeypatch.setenv("COLUMNS", "200")` in this test — the module-level `Console()` at `project_cmds.py:16` wraps output to the detected terminal width, and a narrow width can split an asserted substring across lines. Run it; confirm it fails because the `OSError` propagates.
...
- Whether `git_repo` already creates `.git/hooks` (git normally does) or whether U2 step 1's
  defensive `mkdir` is needed — resolved by running the first test.
