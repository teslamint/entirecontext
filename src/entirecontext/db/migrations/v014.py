"""Migration to schema v14 — widen decision_outcomes.outcome_type CHECK to include 'refined' and 'replaced'."""
...
def _rebuild_decision_outcomes(conn: sqlite3.Connection) -> None:
    exists = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='decision_outcomes'").fetchone()
