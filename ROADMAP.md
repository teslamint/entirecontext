...
- [ ] **`_strip_ec_hooks` drops pre-existing empty hook groups** — when Claude settings hold an unrelated matcher entry whose `hooks` list is already `[]`, `_strip_ec_hooks()` discards the whole entry even though no EntireContext command was removed, so `ec init`/`ec enable`/`ec disable` rewrite the settings without that user-owned matcher. It cannot distinguish "became empty from our filtering" from "was already empty" (`project_cmds.py:348-350`). Reproduced on merged `main` during the 2026-08-12 retro. Regression test must assert a `"hooks": []` matcher survives all three commands, because `_strip_ec_hooks` is called from `project_cmds.py:529` and `:603`. Surfaced by PR #205 review (`3756532937`), posted 20 s before the merge and never processed. _(data loss, P2)_
