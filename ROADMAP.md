...
- [ ] **Plan-vs-spec test enumeration check** — SC2 named `test_init_agent_codex_writes_notify`; the plan's U2 step 2 enumerated seven tests and folded that assertion into a sibling without recording the merge, and every unit acceptance was written against the plan's list rather than the spec's, so all three units reported complete with a declared test absent. Add a planning-phase diff of the plan's test enumeration against the spec's, requiring an inline note for any merged or dropped spec-named test. Surfaced by the 2026-08-12 retro (T11). _(process, P3)_
...
- [ ] **Pre-execute plan verification commands at authoring time** — plan U4 step 5 (PR #217) declared a pyproject `rg` check that "must return no `ignore_errors` context", but `entirecontext.core.cross_repo` has sat in the grandfathered override block since 2026-06-09, so the wording was never satisfiable; every pre-measured check in that plan held, and the single never-run check was the single false one. Require each plan's verification commands to be executed against the current tree during planning, recording the observed output. Surfaced by the 2026-08-12 cross-repo-overload retro (T5). _(process, P3)_
