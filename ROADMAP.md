...
- [ ] **Build-SHA provenance stamp** — the agent hooks invoke `ec` from a uv tool install, not from the checkout. That install predated PR #214, so hook-driven `auto_distill` regenerated `LESSONS.md` with the old code and silently reverted a shipped fix; both copies reported version `0.14.0`, so no version comparison could detect it. Stamp the git SHA at build time and have `ec doctor` compare it against `git rev-parse HEAD`. Surfaced by the 2026-08-12 retro (T8, T14, T15). _(architecture, P3)_
