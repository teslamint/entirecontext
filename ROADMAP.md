...
- [ ] **`disable` deletes empty group keys when sibling group triggers rewrite** — `project_cmds.py:607-610`: when `filtered` is falsy (`[]`), `del hooks[hook_name]` removes the key, and if any sibling group triggers `path_changed = True`, the file is rewritten without the empty group. Same bug class as the `_strip_ec_hooks` empty-group fix (PR #218). Guard with `elif original:` instead of bare `else:`. Surfaced by PR #218 code review. _(data loss, P2)_
