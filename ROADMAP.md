...
- [ ] **Re-query review threads immediately before merge** — the PR #205 merge gate acted on a review snapshot taken 6 minutes earlier, so a P2 comment posted 20 s before the merge was never seen. Add a pre-merge re-query comparing the maximum `comments.created_at` against the last processed round. This narrows the race to a round-trip but cannot close it; the closing fix is a server-side branch-protection rule requiring all review threads resolved. Surfaced by the 2026-08-12 retro (T4, T10). _(process, P2)_
