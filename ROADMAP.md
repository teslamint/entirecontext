...
all; the rest remain open. Four further items were added by the 2026-08-12 retrospective
(`docs/retros/2026-08-12-init-installs-hooks-retro.md`), including one PR #205 review comment
that raced the merge and was never processed. The `@overload` item at the end is unrelated to
hooks — it is typing debt surfaced by PR #215 and given a target version here so it does not
sit unscheduled.
...
- [x] **`--agent codex` installs no git hooks** — fixed by moving the single `_install_git_hooks()` call out of the `claude|both` branch and into the shared integration path. `post-commit` checkpointing and `pre-push` sync now install by default for `claude`, `codex`, and `both`; Claude settings and Codex notify remain agent-specific, and `--no-git-hooks` remains the opt-out. `tests/test_project_cmds.py`: 77 passed. Governed by `docs/specs/2026-08-16-codex-git-hook-installation-design.md` and EC decision `f3dde400-139f-485b-ba39-9c4f5ff11fd3`. _(correctness, P2)_
- [x] **`ec disable` MCP/Codex cleanup symmetry** — every agent mode now removes the agent-neutral EntireContext Git hooks from the current repository; Claude hooks and Codex notify remain agent-specific. The global MCP entry is preserved by default because another repo, agent, or identical manual configuration may use it; explicit `--remove-mcp` removes only a standard EC stdio form while preserving sibling, top-level, and nonstandard configuration. Governed by `docs/specs/2026-08-16-symmetric-disable-lifecycle-design.md`, ADR 0012, and EC decision `30f75661-044d-49e3-a434-b3ca93989be1`; focused lifecycle tests report 7 passed. _(consistency, P3)_
- [x] **Spec directory policy** — `docs/specs/` is the sole active Specification path, matching current practice. Existing Specifications were not moved, and historical archive paths remain preserved. Resolved the prior `docs/superpowers/specs/` policy drift surfaced by PR #205 review (`3755985909`). _(process, P2)_
- [x] **`_strip_ec_hooks` drops pre-existing empty hook groups** — when Claude settings hold an unrelated matcher entry whose `hooks` list is already `[]`, `_strip_ec_hooks()` discards the whole entry even though no EntireContext command was removed, so `ec init`/`ec enable`/`ec disable` rewrite the settings without that user-owned matcher. It cannot distinguish "became empty from our filtering" from "was already empty" (`project_cmds.py:348-350`). Reproduced on merged `main` during the 2026-08-12 retro. Fixed in PR #218: `if not remaining` → `if inner and not remaining`. Regression test and disable-path integration test added. Surfaced by PR #205 review (`3756532937`), posted 20 s before the merge and never processed. _(data loss, P2)_
- [x] **`disable` deletes empty group keys when sibling group triggers rewrite** — `project_cmds.py:607-610`: `else: del hooks[hook_name]` removed keys whose value was `[]` when a sibling group triggered `path_changed = True`. Fixed: `else:` → `elif original:`. Depends on PR #218 for complete coverage. Surfaced by PR #218 code review. _(data loss, P2)_
- [x] **Close the review-thread merge race** — PR #205 merged from a review snapshot taken 6 minutes earlier, so a P2 comment posted 20 s before merge was never processed. Closed on 2026-08-17 by enabling GitHub's server-side `required_conversation_resolution` rule and `enforce_admins` for `main`; the existing required status checks and all other protection settings were preserved. The rule now blocks administrator and non-administrator PR merges until every current review conversation is resolved, so the weaker client-side pre-merge re-query was not added. See [ADR 0016](docs/adr/0016-require-resolved-review-conversations.md). Surfaced by the 2026-08-12 retro (T4, T10). _(process, P2)_
- [x] **Plan-vs-spec test enumeration check** — `scripts/validate_plan.py` now extracts every test identifier from a Specification's `## Testing` section and requires exactly one retained, merged, or dropped row in the Plan's Spec Test Disposition table; merged and dropped rows require rationale. Twelve focused tests cover clean acceptance and every declared disposition, classification, exact-byte, ownership, path/symlink/FIFO, duplicate-key, whole-record, fail-closed, and side-effect-free-validation mutation. Governed by `docs/specs/2026-08-17-planning-contract-enforcement-design.md`, ADR 0013, and EC decision `eb3bc2e9-fe02-44db-9a1f-29cea6ef05a0`. Surfaced by the 2026-08-12 retro (T11). _(process, P3)_
- [x] **Build-SHA provenance stamp** — Hatchling stamps wheel and source-distribution artifacts with the checkout Git SHA and tracked-file dirty state, preserving the stamp when a wheel is rebuilt from an unpacked sdist. When an installed `ec doctor` runs inside an EntireContext checkout, it warns on unavailable, dirty, or mismatched provenance and directs the operator to `uv tool install --force .`; direct source execution and consumer repositories are excluded. Verified by artifact, sdist-rebuild, doctor-boundary, module, and checkout-local smoke tests. Governed by `docs/specs/2026-08-16-build-sha-provenance-design.md`, ADR 0011, and EC decision `edfd67be-253f-46a9-93ce-3b41f37e222e`. Surfaced by the 2026-08-12 retro (T8, T14, T15). _(architecture, P3)_
- [x] **Add `@overload` to the `include_warnings` cross-repo functions** — eleven functions in `src/entirecontext/core/cross_repo.py` return `X | tuple[X, list[WarningEntry]]` depending on the `include_warnings` argument, a shape mypy cannot resolve from a literal argument. Four MCP call sites work around it with `cast` (`tools/search.py`, `tools/session.py` ×2, `tools/checkpoint.py`); every future caller will need the same. Fix by declaring `@overload` variants keyed on `Literal[True]` / `Literal[False]`, then removing the casts. Surfaced while lifting the MCP mypy overrides in PR #215. _(type-safety debt, P3)_ Completed in this cycle: 22 overload stubs across the eleven functions, all four casts removed, mypy enforced at zero.
- [x] **Pre-execute plan verification commands at authoring time** — every governed Plan shell fence is now classified as `plan-check` or rationale-bearing `implementation-only`, and verification commands cannot hide inline; checks must begin with `set -euo pipefail`. The repository tool executes exact LF-delimited check blocks through `record`, preserves combined output bytes, and persists Plan/check-owned status/timestamp plus individual and canonical-record hashes through anchored nonblocking/no-follow operations. `validate` rejects missing, stale, tampered, escaping, symlinked, non-regular, unowned, duplicate-key, masked-failure, and status-mismatch cases without execution. Governed by `docs/specs/2026-08-17-planning-contract-enforcement-design.md`, ADR 0013, and EC decision `eb3bc2e9-fe02-44db-9a1f-29cea6ef05a0`. Surfaced by the 2026-08-12 cross-repo-overload retro (T5). _(process, P3)_
- [ ] **Decide whether to ship `py.typed`** — the package ships no `py.typed` marker, so mypy skips the installed distribution entirely (`import-untyped`) and no external typed caller consumes the public signatures, including the new `include_warnings` overloads. Shipping it would expose the overload contract externally and re-open ADR 0008's bool-fallback question; not shipping it keeps the typed surface internal. Decide deliberately rather than by omission. Surfaced by the 2026-08-12 cross-repo-overload retro (T6); revisit condition recorded in ADR 0008. _(architecture, P4)_
- [ ] **Persist feature-worktree decisions before loop archival** — ADR 0010 published EC decision `0aaa4fa6-6974-4dcf-bf2f-e1ce7d44308b`, but `ec decision show` failed from `main` because the record existed only in the feature worktree. This retro restored current traceability by preserving that original row and ID in the main database; active ADR, Spec, and Plan references remain unchanged. Add a Ship/Retro gate that verifies every active EC decision ID from the base checkout and promotes missing records before worktree removal. _(decision traceability, P2)_
...
- [x] **`get_lessons` flat recency window evicts every expand/narrow lesson** — `src/entirecontext/core/futures.py:142-148` selects `WHERE feedback IS NOT NULL ORDER BY created_at DESC LIMIT ?` with no verdict stratification, so a run of routine neutral assessments evicts the option-shaping lessons the document exists to carry. Measured on the 2026-08-13 regeneration: 50 entries → Expand 1 of 22 surviving, Neutral 49; narrow is absent from the corpus entirely (0 rows), so its missing section is corpus truth rather than eviction. The refresh is self-reinforcing: each assessment *of* the regenerated artifact adds another neutral row that evicts one more expand lesson (assessments `ddcf264d`, `bbd6b204`, `b1302519`, `2a7c4bcd` all report the same collapse). Fixed by reserving a bounded floor *per verdict*, capped at half the total cap, then filling the remaining slots by global recency; `distill_lessons`' section order and empty-section skip are unchanged. Config key `futures.lessons_min_per_verdict` (default 5); 0 restores pure recency. Regenerated 2026-08-15: Expand section 1 -> 5 entries, selection now expand 5 / neutral 45. Surfaced by the repository's own `ec futures distill` dogfooding output, not by review. _(correctness, P2)_
...
- [x] **Decision file rename tracking** — schema v19 persists committed Git rename lineage and a per-repository synchronization watermark; schema v20 persists explicit per-decision path suppressions. `SessionStart` scans only newly reachable commits after the first history scan, materializes destination links transitively into `decision_files`, and leaves historical links intact. A successful file unlink survives replay until an explicit relink, so exact lookup, ranking, outcomes, and stale checks retain their existing contracts without reversing user intent.
...
- **~~Temporal Query Language (TQL)~~** — ✅ **Shipped 2026-07-12.** `--since`/`--until` filters (git refs + ISO dates) on `ec search`, `ec decision search`, `ec decision list` CLI commands and `ec_search`, `ec_decision_search`, `ec_decision_list`, `ec_decision_related` MCP tools. `core/tql.py` provides `resolve_temporal_ref` (git ref → UTC) and `apply_temporal_filters` (datetime()-normalized WHERE injection). Decisions filter on `created_at` (point-in-time existence semantics).
