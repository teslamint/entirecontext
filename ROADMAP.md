...
- [ ] **`get_lessons` flat recency window evicts every expand/narrow lesson** — `src/entirecontext/core/futures.py:142-148` selects `WHERE feedback IS NOT NULL ORDER BY created_at DESC LIMIT ?` with no verdict stratification, so a run of routine neutral assessments evicts the option-shaping lessons the document exists to carry. Measured on the 2026-08-13 regeneration: 50 entries → Expand 1 (and that survivor is a `disagree`-flagged assessment), Narrow 0, Neutral 49, out of a corpus holding 22 expand-verdict assessments. The refresh is self-reinforcing: each assessment *of* the regenerated artifact adds another neutral row that evicts one more expand lesson (assessments `ddcf264d`, `bbd6b204`, `b1302519`, `2a7c4bcd` all report the same collapse). Fix by taking the N most recent *per verdict* rather than N overall, keeping `distill_lessons`' section order and empty-section skip unchanged. Surfaced by the repository's own `ec futures distill` dogfooding output, not by review. _(correctness, P2)_
