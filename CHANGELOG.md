...
- **Mid-session surfacing hook** (#42) — new PostToolUse hook `on_post_tool_use_decisions` surfaces decisions linked to just-edited files. Writes Markdown to `.entirecontext/decisions-context-tooluse-<session>.md` (primary delivery — session-qualified so concurrent sessions in the same repo cannot clobber each other) and also prints to stdout. SessionStart writes its own separate file `.entirecontext/decisions-context.md`. Gated by `decisions.surface_on_tool_use` (default off), with per-turn and session-wide deduplication and rate limiting via `decisions.surface_on_tool_use_turn_interval`. Session-start surfacing now also populates the shared dedup set so decisions shown at session start are not re-surfaced mid-session.
