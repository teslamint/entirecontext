...
- **`ec project merge-worktree <path>`** — merges a legacy per-worktree database (`<linked-worktree>/.entirecontext/db/local.db`, schema v20 or v21) into the canonical project database. The default is a dry run: the merge runs inside a transaction that is rolled back, and no file is copied. `--apply` first snapshots the source and the canonical database with the SQLite backup API under `.entirecontext/backups/`, then copies `content/` files with an md5 check and merges all rows in one transaction. A turn whose content file is missing, fails its md5 check, conflicts with a different canonical file, or points outside `.entirecontext/` keeps its `turns` row but gets no `turn_content` row, and the report lists it. Sessions get the canonical `project_id` and the worktree's `workspace_root`, `worktree_git_dir` and `git_branch`. Rows whose id already exists with different values are reported as divergent and are not overwritten; unique-constraint conflicts are reported as collisions. FTS indexes are updated by their triggers and integrity-checked before commit. After a successful apply, the stale global `repo_index` row for the worktree is removed. The source database is opened read-only and is never modified or deleted.
